...
#
# 用法：
#   build-images.sh tools [CalVer]        # sshd、podman、bind（預設 v2026.8.0）
#
...
tools)
   CALVER="${2:-v2026.8.0}"
   build_push "sshd:${CALVER}"   images/sshd
   build_push "podman:${CALVER}" images/podman
   build_push "bind:${CALVER}"   images/bind
   ;;
*)
